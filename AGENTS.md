...
GNOME Shell extension `gnome-deepseek-usage` — a top-bar indicator for DeepSeek balance. Scaffolded with `create-gnome-extension` (TypeScript). `metadata.json` declares `shell-version: ["50"]`, but the installed `@girs/gnome-shell` type package is v46, so newer shell APIs may lack type definitions.
