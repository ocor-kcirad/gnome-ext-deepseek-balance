...
function build_extension_package() {
	...
	# Compile GSettings schemas, if there are any
	if find "$JS_DIR/schemas" -type f -name '*.gschema.xml' 2> /dev/null | grep -q "."; then
		if command -v glib-compile-schemas &> /dev/null; then
			echo "Compiling GSettings schemas..."
			glib-compile-schemas "$JS_DIR/schemas"
			echo "Schemas compiled."
		else
			echo "ERROR: glib-compile-schemas isn't installed. GSettings schemas won't be compiled. This may cause errors for the extension. Please install glib-compile-schemas and rebuild the extension. Exiting..."
